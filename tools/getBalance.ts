import type { TradingAccount, AccountBalance } from "../src/Quoine/types";
...
import { round, sumBy } from "../src/util";
...
async function main() {
  ...
  if(bfConfig.enabled){
    ...
    const bfJpy = (bfBalance.find(x => x.currency_code === "JPY")).available;
    const bfBtc = (bfBalance.find(x => x.currency_code === "BTC")).available;
    process.stdout.write(`bitFlyer, JPY, Cash, ${round(bfJpy)}\n`);
  ...
  if(ccConfig.enabled){
    ...
    process.stdout.write(`Coincheck, JPY, Cash, ${round(ccBalance.jpy)}\n`);
    ...
    process.stdout.write(`Coincheck, JPY, Margin, ${round(ccLeverageBalance.margin.jpy)}\n`);
    process.stdout.write(`Coincheck, JPY, Free Margin, ${round(ccLeverageBalance.margin_available.jpy)}\n`);
  ...
  if(quConfig.enabled){
    ...
    process.stdout.write(`Quoine, JPY, Cash, ${round(quJpyCash.balance)}\n`);
    ...
    process.stdout.write(`Quoine, JPY, Margin, ${round(quBtcJpyBalance.balance)}\n`);
    process.stdout.write(`Quoine, JPY, Free Margin, ${round(quBtcJpyBalance.free_margin)}\n`);
